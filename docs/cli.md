...
| `locks.unexpected` | Names under `.requivo/locks/` that are not a `<slug>.lock` file `session_lock` could have written — a stray file, a directory, a symlink |
| `locks.unexaminable` | Entries under `.requivo/locks/` whose examination raised — `name` and `error` per entry, on the same terms as `sessions.unexaminable` |
