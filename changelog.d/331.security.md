- #331: the terminal-render sweep in `tests/test_render_untrusted_output.py` only ever ran the six
  ...
  for a `print()`/`input()` call that formats a `Question`'s `q` or `why` field without
  `display_text`/`display_token` -- so a future surface that renders a question is inside the guard
  on the day it is written, not the day someone remembers to add it to a list.
