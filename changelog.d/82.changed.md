...
- Compatibility: breaking - moving a condition from one error code to another, and changing an HTTP
  status, are both listed as breaking in `docs/compatibility.md`. Taken before 1.0 deliberately:
  afterwards it costs a major version, or a promise on that page nobody can keep.
