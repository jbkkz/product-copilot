- A session already on disk under a Windows reserved device name (`con`, `nul`, `com1`...) can be
  ...
  occupies the slug.
- Compatibility: nothing that was refused before is refused now, and nothing that was accepted
  before is rejected. Creating a *new* session at a reserved name is still refused on every
