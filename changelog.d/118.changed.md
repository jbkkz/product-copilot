...
- Both manifests now declare a `$schema`, so an editor validates them as you type and a reader can
  ...
  `displayName` and `repository`, and `homepage` moves from the repository root to the plugin's own
  README: the reference calls `homepage` the documentation URL and gives `repository` to the source,
  and that README is the page a reader wants in the minute after installing.
