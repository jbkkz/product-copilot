...
- `requivo discover`, `demo`, `schema` and `context` no longer die with a raw `UnicodeDecodeError`
  before doing anything. Twelve bundled assets contain characters the platform default cannot decode,
  so on a non-UTF-8 locale the primary verb could not start at all — observed, not reasoned:
