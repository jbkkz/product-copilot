...
- Two things were reported for filing rather than fixed here, and the first has since landed in the
  ...
  #176. Still open: this workflow has no `permissions:` block, and neither do `ci.yml` or
  `secret-scan.yml` — doing one of the three is the same one-file-not-its-neighbour shape this issue
  is about (#178), and the two gate steps and the CLI install step run `claude` unfenced for the same
  reason a fence there has to preserve the exit code that *is* the gate (#177).
