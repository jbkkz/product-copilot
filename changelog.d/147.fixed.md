...
- So the captured output is printed **verbatim inside a `::stop-commands::` fence**, which is the
  ...
  rather than forged. `claude --version` is squashed to one line at capture instead, because it is
  interpolated into lines emitted outside the fence.
...
- Still open and deliberately not in this change: the workflow has no `permissions:` block, and
  neither do `ci.yml` or `secret-scan.yml`. Doing one of the three is the same one-file-not-its-
  neighbour shape this issue is about, so it wants its own change.
