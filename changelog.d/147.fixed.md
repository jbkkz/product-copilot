...
- Still open and deliberately not in this change, both reported for filing rather than fixed here.
  First: the same unanchored `##[` form is reachable in `scripts/plugin_cli_drift.py`, whose
  `_one_line` squashes whitespace only, so a skill directory named `brief##[error]...` forges an
  annotation with no newline at all — which also makes the Windows leg's skip in `_forging_dir`
  unsound, since that platform refuses the newline and not this vector. Fixing it means reopening a
  heavily-argued docstring, a test helper that only checks for a leading `::`, and a claim already
  written into `changelog.d/139.fixed.md`; that is its own change with its own review. Second: this
  workflow has no `permissions:` block, and neither do `ci.yml` or `secret-scan.yml` — doing one of
  the three is the same one-file-not-its-neighbour shape this issue is about.
