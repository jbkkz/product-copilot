...
- Compatibility: an HTTP/1.0 client that sends no `Host` header now gets a 403 where it previously got
  a response. This is deliberate. HTTP/1.1 requires a `Host`, every browser and every ordinary client
