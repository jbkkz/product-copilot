- Applied the repository's own bug-narrative rule -- CLAUDE.md's *Where a bug narrative lives* -- to
  the four worst call-site prose files: `core/persistence.py` (2,344 -> 1,955 lines), `core/context.py`
  (349 -> 292), `services/sessions.py` (723 -> 650) and `core/integrity.py` (451 -> 434), 536 lines of
...
- Compatibility: compatible - no code line, public `--json` payload, session-format key or error code
  changed; `src/` prose is 52.0% -> 50.4% of lines.
