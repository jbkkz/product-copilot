...
- The rest of the surface was swept rather than assumed (#57): `docs/cli.md`, `docs/session-format.md`
  and `plugins/claude-code/REASONING.md` already state that `--revision` is required, and no other
  option on `artifact save` claims a default. A test now reads the rendered help and fails on either
  form this repository writes one in.
